...
-- EMP_TEMP 의 사원들 중에서 급여가 25000 이하인 사원들의 추가수당을
-- 50으로 수정
SELECT * FROM EMP_TEMP;
...
-- DEPT 테이블의 DEPTNO = 40 부서의 DNAME,LOC 를 추출해서 DEPT_TEMP2의 DNAME,LOC 의 수정
...
SET
	DEPTNO = 70 
WHERE
SAL >= (SELECT MAX(SAL) FROM EXAM_EMP WHERE DEPTNO = 50);


-- EXAM_EMP 에 속한 사원 중 60 번 부서의 사원 중에서 입사일이 가장 빠른 사원보다 늦게
...
-- DELETE
-- EXAM_EMPP 에 속한 사원 중 급여 등급이 5인 사원 삭제
DELETE
FROM
	EXAM_EMP
WHERE
...
--       DB 버전에 따라 한글 문자 하나당 2BYTE 할당 OR 3 BYTE 할당
--       NAME VARCHAR2(10) : 홍길동전 => 들어갈 값의 크기가 크다고 오류 남
--           NVACHAR(10), NCHAR() : 10이 바이트 개념이 아니라 문자 길이 자체를 의미  
...
ALTER TABLE MEMBER_TBL MODIFY NATION nchar(4);

INSERT INTO MEMBER_TBL(ID,NAME,ADDR,NATION,EMAIL,AGE)
...
-- 인덱스 : 빠른 검색
-- 1) 자동 생성: 기본키를 설정 시 인덱스로 자동 설정 됨
-- 2) 직접 인덱스 생성
